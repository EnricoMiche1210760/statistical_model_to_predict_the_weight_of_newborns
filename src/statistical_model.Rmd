...
the type of childbirth, subordinate to the age and the number of pregnancies. ??SMOKE?? così al momento non
mi piace
...
type_of_childbird <- dataset_with_class_anni_madre %>%
    group_by(Anni.madre_cl, N.gravidanze) %>%
    reframe(Tipo.parto = Tipo.parto)

anni_madre_cl_distr <- ggplot(data = type_of_childbird,
                              aes(x = N.gravidanze, fill = Tipo.parto, group = factor(Tipo.parto))) +
                        geom_bar(position = "fill") +
                        scale_x_continuous(labels = seq(0, 12, 1),
                                        breaks = seq(0, 12, 1)) +
                        labs(x = "Mother's age", y = "Proportion", fill = "Birth Type") +
                        theme_minimal() +
                        theme(panel.grid.minor = element_blank(), legend.position = "bottom") +
