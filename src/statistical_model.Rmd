...
As we said on paragraph Peso, distribution of "Peso" and "Lunghezza" is not normal. In Figure `r figure.Number <- figure.Number +1; figure.Number`  we can see the Q-Q Plots for the two variables, which show a strong deviation from normality.
```{r qqplot_peso_lunghezza, results='asis', fig.align='center', echo=FALSE, fig.height=5, fig.width=5}
...
caption_text <- textGrob(caption_text, gp = gpar(fontsize = 9), just = "center")
grid.draw(arrangeGrob(qq_plot_peso, qq_plot_lunghezza, bottom = caption_text, nrow = 2))

```

```{r qqplot_peso_lunghezza, results='asis', fig.align='center', echo=FALSE, fig.height=5, fig.width=5}

#posso usare t-student :) per distribuzioni NON normali a differenza del test Z,
# è più rilassato nel rifiutare un ipotesi.

Peso_distr_IQR <- newborns_dataset %>%
  filter(Peso >= quantile(Peso, 0.25) & Peso <= quantile(Peso, 0.75))

t.test(Peso_distr_IQR$Peso, 
       mu = 3300,
       conf.level = 0.95, 
       alternative = "two")

#TEST T con criterio SOGLIA
t.test(newborns_dataset$Peso, 
       mu = 3300,
       conf.level = 0.95, 
       alternative = "two")
...
t.test(newborns_dataset$Lunghezza, 
       mu = 500,
       conf.level = 0.95, 
       alternative = "two")
